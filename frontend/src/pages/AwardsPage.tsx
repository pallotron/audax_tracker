...
export default function AwardsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Audax Ireland Awards ─────────────────────────────────────── */}
      <section className="space-y-3">
        <SectionHeading>Audax Ireland Awards</SectionHeading>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          >
            {easterFleches.length === 0 ? (
              <span className="text-xs text-gray-400 italic">No Easter Flèche completions yet</span>
            ) : (
      ...
      <section className="space-y-3">
        <SectionHeading>International Awards</SectionHeading>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">

          {/* ISR — Randonneurs Mondiaux */}
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="mb-1 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-gray-800">International Super Randonneur (ISR)</h3>
                <p className="text-xs text-gray-400">Randonneurs Mondiaux</p>
              </div>
              {isr.met ? (
                <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800">Completed ✓</span>
              ) : (
                <span className="rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-medium text-yellow-800">In progress</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {(["BRM200", "BRM300", "BRM400", "BRM600"] as const).map((dist) => {
                ...
                return (
                  <div
                    key={dist}
                    className={`rounded border p-2 text-center text-xs ${assignment ? "border-green-300 bg-green-50" : "border-gray-200 bg-gray-50"}`}
                  >
          ...
          {/* 4 Nations SR — Audax Ireland + Audax UK */}
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="mb-1 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-gray-800">4 Nations Super Randonneur</h3>
                <p className="text-xs text-gray-400">Audax Ireland + Audax UK</p>
              </div>
              {fourNations.met ? (
                <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800">Completed ✓</span>
              ) : (
            ...
            <p className="mb-3 text-xs text-gray-500">
              Complete BRM 200+300+400+600 with each distance starting in a different nation (England, Ireland, Scotland, Wales). Counts from 2024–25 season onward.
            </p>
            <div className="grid grid-cols-4 gap-2">
              {(["BRM200", "BRM300", "BRM400", "BRM600"] as const).map((dist) => {
          ...
          {/* International Rides */}
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <h3 className="mb-1 text-sm font-semibold text-gray-800">International Rides</h3>
            <p className="mb-3 text-xs text-gray-500">
              Brevets started outside Ireland, or manually flagged notable international events.
            </p>
            {internationalRides.length === 0 ? (
              <p className="text-xs text-gray-400 italic">No international rides yet. Sync activities to detect rides abroad.</p>
            ) : (
                ...
                {internationalRides.map((a) => (
                  <li key={a.stravaId} className="flex items-center gap-2 text-sm">
                    ...
                    {a.isNotableInternational && (
                      <span className="rounded-full bg-purple-100 px-2 py-0.5 text-xs text-purple-700">notable</span>
                    )}
                  </li>
                ))}
              </ul>
